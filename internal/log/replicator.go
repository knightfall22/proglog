...
type Replicator struct {
	...
	// replicating from a server when the server fails or leaves the cluster.
...
func (r *Replicator) replicate(addr string, leave chan struct{}) {
	...
	records := make(chan *proglog.Record)
	...
	for {
		...
		case <-leave:
			return
		case record := <-records:
			_, err := r.LocalServer.Produce(ctx, &proglog.ProduceRequest{
				Record: record,
			})
			if err != nil {
